...
function [MOF,err] = B1mapOverlayFigure_v1b_Func(MOF,INPUT)
...
if strcmp(DPROPS.hist,'Yes')
    ...
    if strcmp(MOF.type,'dB')
        ...
        xlabel('Relative B1 (Linear)'); ylabel('Voxels');
        title('B1 Histogram (Linear)');
    end
    %xlim([min(test(:)) max(test(:))]);
    xlim([fulldispwid(1) fulldispwid(2)]);
end
