...
function [ROIPLOT,err] = PlotRoiClrSlct_v1a_Func(ROIPLOT,INPUT)
...
%clr = 'rbgcm';
%linewidth = 1;
if strcmp(ROIPLOT.clr,'w')
    ROIPLOT.clr = [0.99 0.99 0.99];
end
for n = 1:length(ROIarr)
    ...
    for m = 1:length(SliceArray)
        ROIarr(n).OutsideOffsetsDrawROI(MSTRCT.Vadd(m),MSTRCT.Hadd(m),SliceArray(m),MSTRCT.ahand,ROIPLOT.clr,ROIPLOT.linewidth);
    end
end
