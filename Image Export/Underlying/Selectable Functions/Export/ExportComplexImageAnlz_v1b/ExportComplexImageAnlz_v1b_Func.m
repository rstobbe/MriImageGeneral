...
function [EXPORT,err] = ExportComplexImageAnlz_v1b_Func(EXPORT,INPUT)
...
[x,y,z] = size(Im);
avw = avw_hdr_make_rob(x,y,z,voxx,voxy,voxz,max(abs(Im(:))),0);      
...
avw.img = real(Im);
avw_img_write(avw,[filename,'_real'],[],'ieee-le',1);
avw.img = imag(Im);
avw_img_write(avw,[filename,'_imag'],[],'ieee-le',1);
